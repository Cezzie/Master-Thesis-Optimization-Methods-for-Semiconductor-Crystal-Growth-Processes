...
class StressDataset(Dataset):
    def __init__(self, data_dir, num_samples, predict_channels=(2)):
        self.data_dir = data_dir
        self.num_samples = num_samples
        self.predict_channels = predict_channels  # Channel 2 is for the stress
        ...
        for i in range(1, num_samples + 1):
            ...
            for t in range(T - 1):
                ...
                input_tensor = data[:, :, 1, t]  # Shape: (r, z, 3)
                # Select the channels to predict (e.g., channel 1)
                target_tensor = data[:, :, 2, t]  # Shape: (r, z, 1) or (r, z)
        ...
        self.inputs = np.stack(self.inputs)   # Shape: (N, r, z, 1)
        self.targets = np.stack(self.targets) # Shape: (N, r, z, 1) or (N, 1, r, z)
        print(f"[Dataset] inputs shape: {self.inputs.shape}, targets shape: {self.targets.shape}")
        # Compute min and max
        min_val = np.min(self.targets)
        max_val = np.max(self.targets)

        print(f"Target min: {min_val}, max: {max_val}")

        # Normalize
        self.targets = (self.targets - min_val) / (max_val - min_val)

        # Optionally, store for denormalization later
    ...
    def __getitem__(self, idx):
        input_tensor = torch.tensor(self.inputs[idx], dtype=torch.float32).permute(2, 0, 1)   # (1, r, z)
...
# Get the total number of samples in the dataset
    ...
    # Assuming your first encoder layer is model.enc1[0]
    in_c = model.enc1[0].in_channels
    out_c = model.final_conv.out_channels if hasattr(model, 'final_conv') else 'Unknown'

    print(f"[Model] Input channels: {in_c}, Output channels: {out_c}")
    print(f"[Model] Input channels: {model.enc1[0].weight.shape}")
    print(f"[Model] Output: {model.out_conv.weight.shape} ")

    return model, train_loader, test_loader, loss_fn, optimizer, device

def train_one_batch(model, data, target, optimizer, loss_fn, device, scaler=None):
...
def train(config):
    ...
    model, train_loader, test_loader, loss_fn, optimizer, device = make_pipeline(config)
    ...
    patience = config.get("early_stopping_patience", 100)  # Stop after 100 epochs of no improvement
    ...
    for epoch in range(config["epochs"]):
        ...
        for data, target in train_loader:
            batch_loss = train_one_batch(model, data, target, optimizer, loss_fn, device, scaler)
        ...
        # Validation loop
        avg_val_loss = validate(model, test_loader, loss_fn, device)
...
# Argument parser, Add arguments if you would like and modify it in the config dictionary accordingly
